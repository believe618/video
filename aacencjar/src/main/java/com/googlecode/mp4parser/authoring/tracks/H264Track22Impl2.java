...
import com.googlecode.mp4parser.h264.model.PictureParameterSet2;
import com.googlecode.mp4parser.h264.model.SeqParameterSet2;
 ...
 * Annex B file.
 */
public class H264Track22Impl2 extends AbstractTrack22 {
    ...
    List<SampleDependencyTypeBox2.Entry> sdtp;
    List<Integer> stss;

    SeqParameterSet2 seqParameterSet2 = null;
    PictureParameterSet2 pictureParameterSet2 = null;
    ...
    public H264Track22Impl2(InputStream inputStream) throws IOException {
        this.reader = new ReaderWrapper(inputStream);
        stts = new LinkedList<TimeToSampleBox2.Entry>();
        ctts = new LinkedList<CompositionTimeToSample2.Entry>();
        sdtp = new LinkedList<SampleDependencyTypeBox2.Entry>();
        ...
        VisualSampleEntry2 visualSampleEntry = new VisualSampleEntry2("avc1");
        ...
        AvcConfigurationBox2 avcConfigurationBox = new AvcConfigurationBox2();
        ...
        avcConfigurationBox.setAvcLevelIndication(seqParameterSet2.level_idc);
        avcConfigurationBox.setAvcProfileIndicaation(seqParameterSet2.profile_idc);
        avcConfigurationBox.setBitDepthLumaMinus8(seqParameterSet2.bit_depth_luma_minus8);
        avcConfigurationBox.setBitDepthChromaMinus8(seqParameterSet2.bit_depth_chroma_minus8);
        avcConfigurationBox.setChromaFormat(seqParameterSet2.chroma_format_idc.getId());
        ...
        trackMetaData2.setTimescale(timescale); // Audio tracks always use samplerate as timescale
        trackMetaData2.setWidth(width);
        trackMetaData2.setHeight(height);
    }
    ...
    public AbstractMediaHeaderBox2 getMediaHeaderBox() {
        return new VideoMediaHeaderBox2();
    }
    ...
    private boolean readVariables() {
        width = (seqParameterSet2.pic_width_in_mbs_minus1 + 1) * 16;
        int mult = 2;
        if (seqParameterSet2.frame_mbs_only_flag) {
            mult = 1;
        }
        height = 16 * (seqParameterSet2.pic_height_in_map_units_minus1 + 1) * mult;
        if (seqParameterSet2.frame_cropping_flag) {
            int chromaArrayType = 0;
            if (seqParameterSet2.residual_color_transform_flag == false) {
                chromaArrayType = seqParameterSet2.chroma_format_idc.getId();
            }
            int cropUnitX = 1;
            int cropUnitY = mult;
            if (chromaArrayType != 0) {
                cropUnitX = seqParameterSet2.chroma_format_idc.getSubWidth();
                cropUnitY = seqParameterSet2.chroma_format_idc.getSubHeight() * mult;
            }

            width -= cropUnitX * (seqParameterSet2.frame_crop_left_offset + seqParameterSet2.frame_crop_right_offset);
            height -= cropUnitY * (seqParameterSet2.frame_crop_top_offset + seqParameterSet2.frame_crop_bottom_offset);
        }
        return true;
    }
        ...
        while (findNextStartcode()) {
            ...
            switch (action) {
                ...
                case STORE:
                    ...
                    SliceHeader sh = new SliceHeader(bs, seqParameterSet2, pictureParameterSet2, IdrPicFlag);
                    ...
                    buffered.clear();
                    samples.add(bb);
                    stts.add(new TimeToSampleBox2.Entry(1, frametick));
                    ...
                    ctts.add(new CompositionTimeToSample2.Entry(1, offset * frametick));
                    sdtp.add(new SampleDependencyTypeBox2.Entry(stdpValue));
                    frameNrInGop++;
                    break;
    ...
    private NALActions handleNALUnit(int nal_ref_idc, int nal_unit_type, byte[] data) throws IOException {
        ...
        switch (nal_unit_type) {
            ...
            case 6:
                seiMessage = new SEIMessage(cleanBuffer(data), seqParameterSet2);
...
//                printAccessUnitDelimiter(data);
            ...
            case 7:
                if (seqParameterSet2 == null) {
                    ByteArrayInputStream is = new ByteArrayInputStream(data);
                    is.read();
                    seqParameterSet2 = SeqParameterSet2.read(is);
                    seqParameterSetList.add(data);
                    if (seqParameterSet2.vuiParams != null) {
                        timescale = seqParameterSet2.vuiParams.time_scale >> 1; // Not sure why, but I found this in several places, and it works...
                        frametick = seqParameterSet2.vuiParams.num_units_in_tick;
                    } else {
            ...
            case 8:
                if (pictureParameterSet2 == null) {
                    ByteArrayInputStream is = new ByteArrayInputStream(data);
                    is.read();
                    pictureParameterSet2 = PictureParameterSet2.read(is);
    ...
    public static class SliceHeader {
        ...
        public SliceHeader(InputStream is, SeqParameterSet2 sps, PictureParameterSet2 pps, boolean IdrPicFlag) throws IOException {
            is.read();
            CAVLCReader2 reader = new CAVLCReader2(is);
    ...
    public class SEIMessage {
        ...
        SeqParameterSet2 sps;

        public SEIMessage(InputStream is, SeqParameterSet2 sps) throws IOException {
            this.sps = sps;
            is.read();
                ...
                if (datasize - read >= payloadSize) {
                    if (payloadType == 1) { // pic_timing is what we are interested in!
                        if (sps.vuiParams != null && (sps.vuiParams.nalHRDParams != null || sps.vuiParams.vclHRDParams != null || sps.vuiParams.pic_struct_present_flag)) {
                            ...
                            is.read(data);
                            read += payloadSize;
                            CAVLCReader2 reader = new CAVLCReader2(new ByteArrayInputStream(data));
