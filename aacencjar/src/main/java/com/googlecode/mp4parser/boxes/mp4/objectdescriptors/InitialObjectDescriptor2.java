...
public class InitialObjectDescriptor2 extends ObjectDescriptorBase2 {
    ...
    List<ESDescriptor2> esDescriptors = new ArrayList<ESDescriptor2>();

    List<ExtensionDescriptor2> extensionDescriptors = new ArrayList<ExtensionDescriptor2>();

    List<BaseDescriptor2> unknownDescriptors = new ArrayList<BaseDescriptor2>();

    @Override
    public void parseDetail(ByteBuffer bb) throws IOException {
        int data = IsoTypeReader2.readUInt16(bb);
        ...
        if (urlFlag == 1) {
            ...
            oDProfileLevelIndication = IsoTypeReader2.readUInt8(bb);
            sceneProfileLevelIndication = IsoTypeReader2.readUInt8(bb);
            audioProfileLevelIndication = IsoTypeReader2.readUInt8(bb);
            visualProfileLevelIndication = IsoTypeReader2.readUInt8(bb);
            graphicsProfileLevelIndication = IsoTypeReader2.readUInt8(bb);

            sizeLeft = sizeLeft - 5;

            if (sizeLeft > 2) {
                final BaseDescriptor2 descriptor = ObjectDescriptorFactory2.createFrom(-1, bb);
                sizeLeft = sizeLeft - descriptor.getSize();
                if (descriptor instanceof ESDescriptor2) {
                    esDescriptors.add((ESDescriptor2) descriptor);
                } else {
        ...
        if (sizeLeft > 2) {
            final BaseDescriptor2 descriptor = ObjectDescriptorFactory2.createFrom(-1, bb);
            if (descriptor instanceof ExtensionDescriptor2) {
                extensionDescriptors.add((ExtensionDescriptor2) descriptor);
            } else {
                unknownDescriptors.add(descriptor);
            }
        }
