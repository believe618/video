 ...
 */
public class TfrfBox2 extends AbstractFullBox2 {
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        IsoTypeWriter2.writeUInt8(byteBuffer, entries.size());

        for (Entry entry : entries) {
            if (getVersion() == 0x01) {
                IsoTypeWriter2.writeUInt64(byteBuffer, entry.fragmentAbsoluteTime);
                IsoTypeWriter2.writeUInt64(byteBuffer, entry.fragmentAbsoluteDuration);
            } else {
                IsoTypeWriter2.writeUInt32(byteBuffer, entry.fragmentAbsoluteTime);
                IsoTypeWriter2.writeUInt32(byteBuffer, entry.fragmentAbsoluteDuration);
            }
        }
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        int fragmentCount = IsoTypeReader2.readUInt8(content);

        for (int i = 0; i < fragmentCount; i++) {
            Entry entry = new Entry();
            if (getVersion() == 0x01) {
                entry.fragmentAbsoluteTime = IsoTypeReader2.readUInt64(content);
                entry.fragmentAbsoluteDuration = IsoTypeReader2.readUInt64(content);
            } else {
                entry.fragmentAbsoluteTime = IsoTypeReader2.readUInt32(content);
                entry.fragmentAbsoluteDuration = IsoTypeReader2.readUInt32(content);
            }
            entries.add(entry);
        }
    }
