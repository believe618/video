 ...
 * }
 */
@Descriptor2(tags = {0x04})
public class DecoderConfigDescriptor2 extends BaseDescriptor2 {
    private static Logger log = Logger.getLogger(DecoderConfigDescriptor2.class.getName());
    ...
    DecoderSpecificInfo2 decoderSpecificInfo2;
    AudioSpecificConfig2 audioSpecificInfo;
    List<ProfileLevelIndicationDescriptor2> profileLevelIndicationDescriptors = new ArrayList<ProfileLevelIndicationDescriptor2>();
    byte[] configDescriptorDeadBytes;

    @Override
    public void parseDetail(ByteBuffer bb) throws IOException {
        objectTypeIndication = IsoTypeReader2.readUInt8(bb);

        int data = IsoTypeReader2.readUInt8(bb);
        streamType = data >>> 2;
        upStream = (data >> 1) & 0x1;

        bufferSizeDB = IsoTypeReader2.readUInt24(bb);
        maxBitRate = IsoTypeReader2.readUInt32(bb);
        avgBitRate = IsoTypeReader2.readUInt32(bb);



        BaseDescriptor2 descriptor;
        if (bb.remaining() > 2) { //1byte tag + at least 1byte size
            final int begin = bb.position();
            descriptor = ObjectDescriptorFactory2.createFrom(objectTypeIndication, bb);
            ...
            if (descriptor instanceof DecoderSpecificInfo2) {
                decoderSpecificInfo2 = (DecoderSpecificInfo2) descriptor;
            }
            if (descriptor instanceof AudioSpecificConfig2) {
                audioSpecificInfo = (AudioSpecificConfig2) descriptor;
            }
        }
        ...
        while (bb.remaining() > 2) {
            ...
            if (descriptor instanceof ProfileLevelIndicationDescriptor2) {
                profileLevelIndicationDescriptors.add((ProfileLevelIndicationDescriptor2) descriptor);
            }
        }
    }
    ...
    public ByteBuffer serialize() {
        ...
        IsoTypeWriter2.writeUInt8(out, 4);
        IsoTypeWriter2.writeUInt8(out, serializedSize() - 2);
        IsoTypeWriter2.writeUInt8(out, objectTypeIndication);
        int flags = (streamType << 2) | (upStream << 1) | 1;
        IsoTypeWriter2.writeUInt8(out, flags);
        IsoTypeWriter2.writeUInt24(out, bufferSizeDB);
        IsoTypeWriter2.writeUInt32(out, maxBitRate);
        IsoTypeWriter2.writeUInt32(out, avgBitRate);
        out.put(audioSpecificInfo.serialize().array());
        return out;
    }

    public DecoderSpecificInfo2 getDecoderSpecificInfo2() {
        return decoderSpecificInfo2;
    }

    public AudioSpecificConfig2 getAudioSpecificInfo() {
        return audioSpecificInfo;
    }

    public void setAudioSpecificInfo(AudioSpecificConfig2 audioSpecificInfo) {
        this.audioSpecificInfo = audioSpecificInfo;
    }

    public List<ProfileLevelIndicationDescriptor2> getProfileLevelIndicationDescriptors() {
    ...
    public String toString() {
        ...
        sb.append(", decoderSpecificInfo=").append(decoderSpecificInfo2);
        sb.append(", audioSpecificInfo=").append(audioSpecificInfo);
        sb.append(", configDescriptorDeadBytes=").append(Hex2.encodeHex(configDescriptorDeadBytes != null ? configDescriptorDeadBytes : new byte[]{}));
