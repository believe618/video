...
@Descriptor2(tags = 0x5, objectTypeIndication = 0x40)
public class AudioSpecificConfig2 extends BaseDescriptor2 {
    ...
    public void parseDetail(ByteBuffer bb) throws IOException {
        ...
        BitReaderBuffer2 bitReaderBuffer2 = new BitReaderBuffer2(configBytes);
        audioObjectType = getAudioObjectType(bitReaderBuffer2);
        samplingFrequencyIndex = bitReaderBuffer2.readBits(4);

        if (samplingFrequencyIndex == 0xf) {
            samplingFrequency = bitReaderBuffer2.readBits(24);
        }

        channelConfiguration = bitReaderBuffer2.readBits(4);
        ...
        if (audioObjectType == 5 ||
            ...
            extensionSamplingFrequencyIndex = bitReaderBuffer2.readBits(4);
            if (extensionSamplingFrequencyIndex == 0xf)
                extensionSamplingFrequency = bitReaderBuffer2.readBits(24);
            audioObjectType = getAudioObjectType(bitReaderBuffer2);
            if (audioObjectType == 22)
                extensionChannelConfiguration = bitReaderBuffer2.readBits(4);
        } else {
            extensionAudioObjectType = 0;
        }
        ...
        switch (audioObjectType) {
            ...
            case 22:
            case 23:
                parseGaSpecificConfig(samplingFrequencyIndex, channelConfiguration, audioObjectType, bitReaderBuffer2);
                //GASpecificConfig();
                break;
            case 8:
            ...
            case 26:
            case 27:
                parseParametricSpecificConfig(samplingFrequencyIndex, channelConfiguration, audioObjectType, bitReaderBuffer2);
                //ParametricSpecificConfig();
                break;
            case 28:
                ...
                //break;
            case 30:
                sacPayloadEmbedding = bitReaderBuffer2.readBits(1);
            ...
            case 35:
                ...
                //break;
            case 36:
                fillBits = bitReaderBuffer2.readBits(5);
        ...
        switch (audioObjectType) {
            ...
            case 27:
            case 39:
                epConfig = bitReaderBuffer2.readBits(2);
                if (epConfig == 2 || epConfig == 3) {
                    throw new UnsupportedOperationException("can't parse ErrorProtectionSpecificConfig yet");
                    //ErrorProtectionSpecificConfig();
                }
                if (epConfig == 3) {
                    directMapping = bitReaderBuffer2.readBits(1);
                    if (directMapping == 0) {
                        /* tbd */
        ...
        if (extensionAudioObjectType != 5 && bitReaderBuffer2.remainingBits() >= 16) {
            syncExtensionType = bitReaderBuffer2.readBits(11);
            if (syncExtensionType == 0x2b7) {
                extensionAudioObjectType = getAudioObjectType(bitReaderBuffer2);
                if (extensionAudioObjectType == 5) {
                    sbrPresentFlag = bitReaderBuffer2.readBits(1);
                    if (sbrPresentFlag == 1) {
                        extensionSamplingFrequencyIndex = bitReaderBuffer2.readBits(4);
                        if (extensionSamplingFrequencyIndex == 0xf) {
                            extensionSamplingFrequency = bitReaderBuffer2.readBits(24);
                        }
                        if (bitReaderBuffer2.remainingBits() >= 12) {
                            syncExtensionType = bitReaderBuffer2.readBits(11); //10101001000
                            if (syncExtensionType == 0x548) {
                                psPresentFlag = bitReaderBuffer2.readBits(1);
                            }
                        }
                    }
                }
    ...
    public ByteBuffer serialize() {
        ...
        IsoTypeWriter2.writeUInt8(out, 5);
        IsoTypeWriter2.writeUInt8(out, serializedSize() - 2);
        BitWriterBuffer2 bwb = new BitWriterBuffer2(out);
    ...
    private int getAudioObjectType(BitReaderBuffer2 in) throws IOException {
    ...
    private void parseGaSpecificConfig(int samplingFrequencyIndex, int channelConfiguration, int audioObjectType, BitReaderBuffer2 in) throws IOException {
...
//    }
    ...
    private void parseParametricSpecificConfig(int samplingFrequencyIndex, int channelConfiguration, int audioObjectType, BitReaderBuffer2 in) throws IOException {
        /*
        ParametricSpecificConfig() {
    ...
    private void parseParaConfig(int samplingFrequencyIndex, int channelConfiguration, int audioObjectType, BitReaderBuffer2 in) throws IOException {
        /*
        PARAconfig()
        {
            PARAmode; 2 uimsbf
    ...
    private void parseErHvxcConfig(int samplingFrequencyIndex, int channelConfiguration, int audioObjectType, BitReaderBuffer2 in) throws IOException {
        /*
        ErHVXCconfig()
        {
    ...
    private void parseHilnConfig(int samplingFrequencyIndex, int channelConfiguration, int audioObjectType, BitReaderBuffer2 in) throws IOException {
        /*
        HILNconfig()
        {
    ...
    private void parseHilnEnexConfig(int samplingFrequencyIndex, int channelConfiguration, int audioObjectType, BitReaderBuffer2 in) throws IOException {
        /*
        HILNenexConfig()
        {
