...
public abstract class AbstractSampleEncryptionBox2 extends AbstractFullBox2 {
    int algorithmId = -1;
    int ivSize = -1;
    byte[] kid = new byte[16];
    List<Entry> entries = new LinkedList<Entry>();

    protected AbstractSampleEncryptionBox2(String type) {
        super(type);
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        if ((getFlags() & 0x1) > 0) {
            algorithmId = IsoTypeReader2.readUInt24(content);
            ivSize = IsoTypeReader2.readUInt8(content);
            kid = new byte[16];
            content.get(kid);
        }
        long numOfEntries = IsoTypeReader2.readUInt32(content);
        while (numOfEntries-- > 0) {
            Entry e = new Entry();
            e.iv = new byte[((getFlags() & 0x1) > 0) ? ivSize : 8];
            content.get(e.iv);
            if ((getFlags() & 0x2) > 0) {
                int numOfPairs = IsoTypeReader2.readUInt16(content);
                e.pairs = new LinkedList<Entry.Pair>();
                while (numOfPairs-- > 0) {
                    e.pairs.add(e.createPair(IsoTypeReader2.readUInt16(content), IsoTypeReader2.readUInt32(content)));
                }
            }
            entries.add(e);
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        if (isOverrideTrackEncryptionBoxParameters()) {
            IsoTypeWriter2.writeUInt24(byteBuffer, algorithmId);
            IsoTypeWriter2.writeUInt8(byteBuffer, ivSize);
            byteBuffer.put(kid);
        }
        ...
        for (Entry entry : entries) {
            ...
            if (isSubSampleEncryption()) {
                IsoTypeWriter2.writeUInt16(byteBuffer, entry.pairs.size());
                for (Entry.Pair pair : entry.pairs) {
                    IsoTypeWriter2.writeUInt16(byteBuffer, pair.clear);
                    IsoTypeWriter2.writeUInt32(byteBuffer, pair.encrypted);
                }
            }
        }
    }
    ...
    public boolean equals(Object o) {
        ...
        AbstractSampleEncryptionBox2 that = (AbstractSampleEncryptionBox2) o;
