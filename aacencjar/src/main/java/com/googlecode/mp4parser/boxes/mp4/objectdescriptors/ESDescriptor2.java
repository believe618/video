 ...
 */
@Descriptor2(tags = {0x03})
public class ESDescriptor2 extends BaseDescriptor2 {
    private static Logger log = Logger.getLogger(ESDescriptor2.class.getName());
    ...
    DecoderConfigDescriptor2 decoderConfigDescriptor;
    SLConfigDescriptor2 slConfigDescriptor;
    List<BaseDescriptor2> otherDescriptors = new ArrayList<BaseDescriptor2>();

    @Override
    public void parseDetail(ByteBuffer bb) throws IOException {
        esId = IsoTypeReader2.readUInt16(bb);
        ...
        if (streamDependenceFlag == 1) {
            dependsOnEsId = IsoTypeReader2.readUInt16(bb);
        }
        if (URLFlag == 1) {
            URLLength = IsoTypeReader2.readUInt8(bb);
            URLString = IsoTypeReader2.readString(bb, URLLength);
        }
        if (oCRstreamFlag == 1) {
            oCREsId = IsoTypeReader2.readUInt16(bb);
        }
        ...
        if (getSize() > baseSize + 2) {
            BaseDescriptor2 descriptor = ObjectDescriptorFactory2.createFrom(-1, bb);
            ...
            if (descriptor instanceof DecoderConfigDescriptor2) {
                decoderConfigDescriptor = (DecoderConfigDescriptor2) descriptor;
            }
        }
        ...
        if (getSize() > baseSize + 2) {
            ...
            if (descriptor instanceof SLConfigDescriptor2) {
                slConfigDescriptor = (SLConfigDescriptor2) descriptor;
            }
        } else {
    ...
    public ByteBuffer serialize() {
        ...
        IsoTypeWriter2.writeUInt8(out, 3);
        IsoTypeWriter2.writeUInt8(out, serializedSize() - 2); // Not OK for longer sizes!
        IsoTypeWriter2.writeUInt16(out, esId);
        int flags = (streamDependenceFlag << 7) | (URLFlag << 6) | (oCRstreamFlag << 5) | (streamPriority & 0x1f);
        IsoTypeWriter2.writeUInt8(out, flags);
        if (streamDependenceFlag > 0) {
            IsoTypeWriter2.writeUInt16(out, dependsOnEsId);
        }
        if (URLFlag > 0) {
            IsoTypeWriter2.writeUInt8(out, URLLength);
            IsoTypeWriter2.writeUtf8String(out, URLString);
        }
        if (oCRstreamFlag > 0) {
            IsoTypeWriter2.writeUInt16(out, oCREsId);
        }
...
//  }
    ...
    public DecoderConfigDescriptor2 getDecoderConfigDescriptor() {
        return decoderConfigDescriptor;
    }

    public SLConfigDescriptor2 getSlConfigDescriptor() {
        return slConfigDescriptor;
    }

    public void setDecoderConfigDescriptor(DecoderConfigDescriptor2 decoderConfigDescriptor) {
        this.decoderConfigDescriptor = decoderConfigDescriptor;
    }

    public void setSlConfigDescriptor(SLConfigDescriptor2 slConfigDescriptor) {
        this.slConfigDescriptor = slConfigDescriptor;
    }

    public List<BaseDescriptor2> getOtherDescriptors() {
