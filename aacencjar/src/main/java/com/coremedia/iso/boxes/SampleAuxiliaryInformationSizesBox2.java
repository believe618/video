...
public class SampleAuxiliaryInformationSizesBox2 extends AbstractFullBox2 {
    ...
    public SampleAuxiliaryInformationSizesBox2() {
        super(TYPE);
    }
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        if ((getFlags() & 1) == 1) {
            byteBuffer.put(IsoFile2.fourCCtoBytes(auxInfoType));
            byteBuffer.put(IsoFile2.fourCCtoBytes(auxInfoTypeParameter));
        }

        IsoTypeWriter2.writeUInt8(byteBuffer, defaultSampleInfoSize);
        if (defaultSampleInfoSize == 0) {
            IsoTypeWriter2.writeUInt32(byteBuffer, sampleInfoSizes.size());
            for (short sampleInfoSize : sampleInfoSizes) {
                IsoTypeWriter2.writeUInt8(byteBuffer, sampleInfoSize);
            }
        } else {
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        if ((getFlags() & 1) == 1) {
            auxInfoType = IsoTypeReader2.read4cc(content);
            auxInfoTypeParameter = IsoTypeReader2.read4cc(content);
        }

        defaultSampleInfoSize = (short) IsoTypeReader2.readUInt8(content);
        int sampleCount = l2i(IsoTypeReader2.readUInt32(content));

        sampleInfoSizes.clear();

        for (int i = 0; i < sampleCount; i++) {
            sampleInfoSizes.add((short) IsoTypeReader2.readUInt8(content));
        }
    }
