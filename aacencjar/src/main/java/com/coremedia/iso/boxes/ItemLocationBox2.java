...
import com.coremedia.iso.IsoTypeReaderVariable2;
import com.coremedia.iso.IsoTypeWriter2;
import com.coremedia.iso.IsoTypeWriterVariable2;
 ...
 * }
 */
public class ItemLocationBox2 extends AbstractFullBox2 {
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        IsoTypeWriter2.writeUInt8(byteBuffer, ((offsetSize << 4) | lengthSize));
        if (getVersion() == 1) {
            IsoTypeWriter2.writeUInt8(byteBuffer, (baseOffsetSize << 4 | indexSize));
        } else {
            IsoTypeWriter2.writeUInt8(byteBuffer, (baseOffsetSize << 4));
        }
        IsoTypeWriter2.writeUInt16(byteBuffer, items.size());
        for (Item item : items) {
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        int tmp = IsoTypeReader2.readUInt8(content);
        offsetSize = tmp >>> 4;
        lengthSize = tmp & 0xf;
        tmp = IsoTypeReader2.readUInt8(content);
        baseOffsetSize = tmp >>> 4;

        if (getVersion() == 1) {
            indexSize = tmp & 0xf;
        }
        int itemCount = IsoTypeReader2.readUInt16(content);
    ...
    public class Item {
        ...
        public Item(ByteBuffer in) {
            itemId = IsoTypeReader2.readUInt16(in);

            if (getVersion() == 1) {
                int tmp = IsoTypeReader2.readUInt16(in);
                constructionMethod = tmp & 0xf;
            }

            dataReferenceIndex = IsoTypeReader2.readUInt16(in);
            if (baseOffsetSize > 0) {
                baseOffset = IsoTypeReaderVariable2.read(in, baseOffsetSize);
            } else {
                baseOffset = 0;
            }
            int extentCount = IsoTypeReader2.readUInt16(in);
        ...
        public void getContent(ByteBuffer bb)  {
            IsoTypeWriter2.writeUInt16(bb, itemId);

            if (getVersion() == 1) {
                IsoTypeWriter2.writeUInt16(bb, constructionMethod);
            }


            IsoTypeWriter2.writeUInt16(bb, dataReferenceIndex);
            if (baseOffsetSize > 0) {
                IsoTypeWriterVariable2.write(baseOffset, bb, baseOffsetSize);
            }
            IsoTypeWriter2.writeUInt16(bb, extents.size());
    ...
    public class Extent {
        ...
        public Extent(ByteBuffer in) {
            if ((getVersion() == 1) && indexSize > 0) {
                extentIndex = IsoTypeReaderVariable2.read(in, indexSize);
            }
            extentOffset = IsoTypeReaderVariable2.read(in, offsetSize);
            extentLength = IsoTypeReaderVariable2.read(in, lengthSize);
        }

        public void getContent(ByteBuffer os)  {
            if ((getVersion() == 1) && indexSize > 0) {
                IsoTypeWriterVariable2.write(extentIndex, os, indexSize);
            }
            IsoTypeWriterVariable2.write(extentOffset, os, offsetSize);
            IsoTypeWriterVariable2.write(extentLength, os, lengthSize);
        }
