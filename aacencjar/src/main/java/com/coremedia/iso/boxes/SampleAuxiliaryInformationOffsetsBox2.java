 ...
 */
public class SampleAuxiliaryInformationOffsetsBox2 extends AbstractFullBox2 {
    ...
    public SampleAuxiliaryInformationOffsetsBox2() {
        super(TYPE);
    }
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        if ((getFlags() & 1) == 1) {
            IsoTypeWriter2.writeUInt32(byteBuffer, auxInfoType);
            IsoTypeWriter2.writeUInt32(byteBuffer, auxInfoTypeParameter);
        }

        IsoTypeWriter2.writeUInt32(byteBuffer, offsets.size());
        for (Long offset : offsets) {
            if (getVersion() == 0) {
                IsoTypeWriter2.writeUInt32(byteBuffer, offset);
            } else {
                IsoTypeWriter2.writeUInt64(byteBuffer, offset);
            }
        }
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        if ((getFlags() & 1) == 1) {
            auxInfoType = IsoTypeReader2.readUInt32(content);
            auxInfoTypeParameter = IsoTypeReader2.readUInt32(content);
        }
        ...
        for (int i = 0; i < entryCount; i++) {
            if (getVersion() == 0) {
                offsets.add(IsoTypeReader2.readUInt32(content));
            } else {
                offsets.add(IsoTypeReader2.readUInt64(content));
            }
        }
    }
