...
public class TrackRunBox2 extends AbstractFullBox2 {
    public static final String TYPE = "trun";
    private int dataOffset;
    private SampleFlags2 firstSampleFlags2;
    ...
    public static class Entry {
        ...
        private SampleFlags2 sampleFlags2;
        private int sampleCompositionTimeOffset;

        public Entry() {
        }

        public Entry(long sampleDuration, long sampleSize, SampleFlags2 sampleFlags2, int sampleCompositionTimeOffset) {
            this.sampleDuration = sampleDuration;
            this.sampleSize = sampleSize;
            this.sampleFlags2 = sampleFlags2;
        ...
        public SampleFlags2 getSampleFlags2() {
            return sampleFlags2;
        }
        ...
        public void setSampleFlags2(SampleFlags2 sampleFlags2) {
            this.sampleFlags2 = sampleFlags2;
        }
    ...
    public long[] getSampleOffsets() {
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            if (isSampleSizePresent()) {
                ...
                offset += ((TrackFragmentBox2) getParent()).getTrackFragmentHeaderBox().getDefaultSampleSize();
            }
        }
    ...
    public long[] getSampleSizes() {
        ...
        for (int i = 0; i < result.length; i++) {
            if (isSampleSizePresent()) {
                ...
                result[i] = ((TrackFragmentBox2) getParent()).getTrackFragmentHeaderBox().getDefaultSampleSize();
            }
        }
    ...
    public long[] getSampleDurations() {
        ...
        for (int i = 0; i < result.length; i++) {
            if (isSampleDurationPresent()) {
                ...
                result[i] = ((TrackFragmentBox2) getParent()).getTrackFragmentHeaderBox().getDefaultSampleDuration();
            }
        }
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        if ((flags & 0x1) == 1) { //dataOffsetPresent
            IsoTypeWriter2.writeUInt32(byteBuffer, dataOffset);
        }
        if ((flags & 0x4) == 0x4) { //firstSampleFlagsPresent
            firstSampleFlags2.getContent(byteBuffer);
        }

        for (Entry entry : entries) {
            if ((flags & 0x100) == 0x100) { //sampleDurationPresent
                IsoTypeWriter2.writeUInt32(byteBuffer, entry.sampleDuration);
            }
            if ((flags & 0x200) == 0x200) { //sampleSizePresent
                IsoTypeWriter2.writeUInt32(byteBuffer, entry.sampleSize);
            }
            if ((flags & 0x400) == 0x400) { //sampleFlagsPresent
                entry.sampleFlags2.getContent(byteBuffer);
            }
            if ((flags & 0x800) == 0x800) { //sampleCompositionTimeOffsetPresent
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        long sampleCount = IsoTypeReader2.readUInt32(content);

        if ((getFlags() & 0x1) == 1) { //dataOffsetPresent
            dataOffset = l2i(IsoTypeReader2.readUInt32(content));
        } else {
            dataOffset = -1;
        }
        if ((getFlags() & 0x4) == 0x4) { //firstSampleFlagsPresent
            firstSampleFlags2 = new SampleFlags2(content);
        }

        for (int i = 0; i < sampleCount; i++) {
            Entry entry = new Entry();
            if ((getFlags() & 0x100) == 0x100) { //sampleDurationPresent
                entry.sampleDuration = IsoTypeReader2.readUInt32(content);
            }
            if ((getFlags() & 0x200) == 0x200) { //sampleSizePresent
                entry.sampleSize = IsoTypeReader2.readUInt32(content);
            }
            if ((getFlags() & 0x400) == 0x400) { //sampleFlagsPresent
                entry.sampleFlags2 = new SampleFlags2(content);
            }
    ...
    public SampleFlags2 getFirstSampleFlags2() {
        return firstSampleFlags2;
    }

    public void setFirstSampleFlags2(SampleFlags2 firstSampleFlags2) {
        if (firstSampleFlags2 == null) {
            setFlags(getFlags() & (0xFFFFFF ^ 0x4));
        } else {
            setFlags(getFlags() | 0x4);
        }
        this.firstSampleFlags2 = firstSampleFlags2;
    }
    ...
    public String toString() {
        ...
        sb.append(", firstSampleFlags=").append(firstSampleFlags2);
        sb.append('}');
        return sb.toString();
    }
