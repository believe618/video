 ...
 * }
 */
public class TrackFragmentRandomAccessBox2 extends AbstractFullBox2 {
    ...
    public TrackFragmentRandomAccessBox2() {
        super(TYPE);
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        for (int i = 0; i < numberOfEntries; i++) {
            ...
            if (getVersion() == 1) {
                entry.time = IsoTypeReader2.readUInt64(content);
                entry.moofOffset = IsoTypeReader2.readUInt64(content);
            } else {
                entry.time = IsoTypeReader2.readUInt32(content);
                entry.moofOffset = IsoTypeReader2.readUInt32(content);
            }
            entry.trafNumber = IsoTypeReaderVariable2.read(content, lengthSizeOfTrafNum);
            entry.trunNumber = IsoTypeReaderVariable2.read(content, lengthSizeOfTrunNum);
            entry.sampleNumber = IsoTypeReaderVariable2.read(content, lengthSizeOfSampleNum);
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        IsoTypeWriter2.writeUInt32(byteBuffer, temp);
        IsoTypeWriter2.writeUInt32(byteBuffer, entries.size());

        for (Entry entry : entries) {
            if (getVersion() == 1) {
                IsoTypeWriter2.writeUInt64(byteBuffer, entry.time);
                IsoTypeWriter2.writeUInt64(byteBuffer, entry.moofOffset);
            } else {
                IsoTypeWriter2.writeUInt32(byteBuffer, entry.time);
                IsoTypeWriter2.writeUInt32(byteBuffer, entry.moofOffset);
            }
            IsoTypeWriterVariable2.write(entry.trafNumber, byteBuffer, lengthSizeOfTrafNum);
            IsoTypeWriterVariable2.write(entry.trunNumber, byteBuffer, lengthSizeOfTrunNum);
            IsoTypeWriterVariable2.write(entry.sampleNumber, byteBuffer, lengthSizeOfSampleNum);
