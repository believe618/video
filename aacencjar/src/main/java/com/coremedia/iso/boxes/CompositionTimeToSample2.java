 ...
 */
public class CompositionTimeToSample2 extends AbstractFullBox2 {
    public static final String TYPE = "ctts";

    List<Entry> entries = Collections.emptyList();

    public CompositionTimeToSample2() {
        super(TYPE);
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        int numberOfEntries = l2i(IsoTypeReader2.readUInt32(content));
        entries = new ArrayList<Entry>(numberOfEntries);
        for (int i = 0; i < numberOfEntries; i++) {
            Entry e = new Entry(l2i(IsoTypeReader2.readUInt32(content)), content.getInt());
            entries.add(e);
        }
    }
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        for (Entry entry : entries) {
            IsoTypeWriter2.writeUInt32(byteBuffer, entry.getCount());
     ...
     */
    public static int[] blowupCompositionTimes(List<CompositionTimeToSample2.Entry> entries) {
        long numOfSamples = 0;
        for (CompositionTimeToSample2.Entry entry : entries) {
