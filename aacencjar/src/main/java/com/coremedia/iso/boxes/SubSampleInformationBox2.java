 ...
 * }
 */
public class SubSampleInformationBox2 extends AbstractFullBox2 {
    public static final String TYPE = "subs";

    private long entryCount;
    private List<SampleEntry> entries = new ArrayList<SampleEntry>();

    public SubSampleInformationBox2() {
        super(TYPE);
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        for (int i = 0; i < entryCount; i++) {
            ...
            sampleEntry.setSampleDelta(IsoTypeReader2.readUInt32(content));
            int subsampleCount = IsoTypeReader2.readUInt16(content);
            for (int j = 0; j < subsampleCount; j++) {
                SampleEntry.SubsampleEntry subsampleEntry = new SampleEntry.SubsampleEntry();
                subsampleEntry.setSubsampleSize(getVersion() == 1 ? IsoTypeReader2.readUInt32(content) : IsoTypeReader2.readUInt16(content));
                subsampleEntry.setSubsamplePriority(IsoTypeReader2.readUInt8(content));
                subsampleEntry.setDiscardable(IsoTypeReader2.readUInt8(content));
                subsampleEntry.setReserved(IsoTypeReader2.readUInt32(content));
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        for (SampleEntry sampleEntry : entries) {
            IsoTypeWriter2.writeUInt32(byteBuffer, sampleEntry.getSampleDelta());
            IsoTypeWriter2.writeUInt16(byteBuffer, sampleEntry.getSubsampleCount());
            ...
            for (SampleEntry.SubsampleEntry subsampleEntry : subsampleEntries) {
                if (getVersion() == 1) {
                    IsoTypeWriter2.writeUInt32(byteBuffer, subsampleEntry.getSubsampleSize());
                } else {
                    IsoTypeWriter2.writeUInt16(byteBuffer, l2i(subsampleEntry.getSubsampleSize()));
                }
                IsoTypeWriter2.writeUInt8(byteBuffer, subsampleEntry.getSubsamplePriority());
                IsoTypeWriter2.writeUInt8(byteBuffer, subsampleEntry.getDiscardable());
                IsoTypeWriter2.writeUInt32(byteBuffer, subsampleEntry.getReserved());
            }
        }
    }
