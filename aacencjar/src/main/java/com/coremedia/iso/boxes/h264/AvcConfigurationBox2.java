 ...
 */
public final class AvcConfigurationBox2 extends AbstractBox2 {
    ...
    public void _parseDetails(ByteBuffer content) {
        configurationVersion = IsoTypeReader2.readUInt8(content);
        avcProfileIndicaation = IsoTypeReader2.readUInt8(content);
        profileCompatibility = IsoTypeReader2.readUInt8(content);
        avcLevelIndication = IsoTypeReader2.readUInt8(content);
        int temp = IsoTypeReader2.readUInt8(content);
        lengthSizeMinusOne = temp & 3;
        long numberOfSeuqenceParameterSets = IsoTypeReader2.readUInt8(content) & 31;
        for (int i = 0; i < numberOfSeuqenceParameterSets; i++) {
            int sequenceParameterSetLength = IsoTypeReader2.readUInt16(content);
        ...
        long numberOfPictureParameterSets = IsoTypeReader2.readUInt8(content);
        for (int i = 0; i < numberOfPictureParameterSets; i++) {
            int pictureParameterSetLength = IsoTypeReader2.readUInt16(content);
        ...
        if (hasExts && (avcProfileIndicaation == 100 || avcProfileIndicaation == 110 || avcProfileIndicaation == 122 || avcProfileIndicaation == 144)) {
            chromaFormat = IsoTypeReader2.readUInt8(content) & 3;
            bitDepthLumaMinus8 = IsoTypeReader2.readUInt8(content) & 7;
            bitDepthChromaMinus8 = IsoTypeReader2.readUInt8(content) & 7;
            long numOfSequenceParameterSetExt = IsoTypeReader2.readUInt8(content);
            for (int i = 0; i < numOfSequenceParameterSetExt; i++) {
                int sequenceParameterSetExtLength = IsoTypeReader2.readUInt16(content);
    ...
    public void getContent(ByteBuffer byteBuffer) {
        IsoTypeWriter2.writeUInt8(byteBuffer, configurationVersion);
        IsoTypeWriter2.writeUInt8(byteBuffer, avcProfileIndicaation);
        IsoTypeWriter2.writeUInt8(byteBuffer, profileCompatibility);
        IsoTypeWriter2.writeUInt8(byteBuffer, avcLevelIndication);
        IsoTypeWriter2.writeUInt8(byteBuffer, lengthSizeMinusOne | (63 << 2));
        IsoTypeWriter2.writeUInt8(byteBuffer, (pictureParameterSets.size() & 31) | (7 << 5));
        for (byte[] sequenceParameterSetNALUnit : sequenceParameterSets) {
            IsoTypeWriter2.writeUInt16(byteBuffer, sequenceParameterSetNALUnit.length);
            byteBuffer.put(sequenceParameterSetNALUnit);
        }
        IsoTypeWriter2.writeUInt8(byteBuffer, pictureParameterSets.size());
        for (byte[] pictureParameterSetNALUnit : pictureParameterSets) {
            IsoTypeWriter2.writeUInt16(byteBuffer, pictureParameterSetNALUnit.length);
            byteBuffer.put(pictureParameterSetNALUnit);
        }
        if (hasExts && (avcProfileIndicaation == 100 || avcProfileIndicaation == 110 || avcProfileIndicaation == 122 || avcProfileIndicaation == 144)) {
            IsoTypeWriter2.writeUInt8(byteBuffer, chromaFormat | (63 << 2));
            IsoTypeWriter2.writeUInt8(byteBuffer, bitDepthLumaMinus8 | (31 << 3));
            IsoTypeWriter2.writeUInt8(byteBuffer, bitDepthChromaMinus8 | (31 << 3));
            IsoTypeWriter2.writeUInt8(byteBuffer, sequenceParameterSetExts.size());
            for (byte[] sequenceParameterSetExtNALUnit : sequenceParameterSetExts) {
                IsoTypeWriter2.writeUInt16(byteBuffer, sequenceParameterSetExtNALUnit.length);
    ...
    public String[] getPPS() {
        ...
        for (byte[] pictureParameterSet : pictureParameterSets) {
            ...
            try {
                details = PictureParameterSet2.read(pictureParameterSet).toString();
            } catch (IOException e) {
    ...
    public String[] getSPS() {
        ...
        for (byte[] sequenceParameterSet : sequenceParameterSets) {
            ...
            try {
                detail = SeqParameterSet2.read(new ByteArrayInputStream(sequenceParameterSet)).toString();
            } catch (IOException e) {
