 ...
 * @see UserDataBox2
 */
public class AlbumBox2 extends AbstractFullBox2 {
    ...
    protected long getContentSize() {
        return 6 + Utf82.utf8StringLengthInBytes(albumTitle) + 1 + (trackNumber == -1 ? 0 : 1);
    }
    ...
    public void _parseDetails(ByteBuffer content) {
        ...
        albumTitle = IsoTypeReader2.readString(content);

        if (content.remaining() > 0) {
            trackNumber = IsoTypeReader2.readUInt8(content);
        } else {
            trackNumber = -1;
        }
    }
    ...
    protected void getContent(ByteBuffer byteBuffer) {
        ...
        byteBuffer.put(Utf82.convert(albumTitle));
        byteBuffer.put((byte) 0);
        if (trackNumber != -1) {
            IsoTypeWriter2.writeUInt8(byteBuffer, trackNumber);
        }
    }
